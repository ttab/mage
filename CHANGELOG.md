...
default, because a new service is Connect only. The OpenAPI 3 specifications
follow Twirp: `protoc-gen-openapi3` documents the Twirp surface, down to the
`/twirp/<pkg>.<Service>/<Method>` paths and Twirp's error schema, so a
Connect-only repository writes none rather than committing a specification of
an API it does not serve. `rpc.OpenAPI = rpc.OpenAPIOn` writes them anyway and
`rpc.OpenAPIOff` stops writing them while Twirp is still served. The
version stamped into a specification is resolved from the last ancestor git
tag, and only when a specification is written, so a repository that has not
been tagged yet — which is where a new Connect-only service starts — can
still run `rpc:generate`.
...
`service.twirp.go` files change only in their gzipped descriptor blob, and the
OpenAPI specifications come out byte-identical to the ones the image produced.
Expect one large, mechanical diff per repository and nothing behind it.
...
- New targets `rpc:generate`, `rpc:release`, `rpc:vendorProto` and `rpc:stub`.
  Configuration is the exported variables of the `rpc` package — `rpc.Twirp`,
  `rpc.OpenAPI`, `rpc.VendorDir`, `rpc.ExtraProtoRoots` and
...
- The module now requires `google.golang.org/protobuf`. The OpenAPI generator
  is built against a protobuf-go that reports `FEATURE_SUPPORTS_EDITIONS`
  without a minimum edition, which protoc accepts and buf rejects outright, so
  it is driven directly from the target with the descriptors buf compiled.
